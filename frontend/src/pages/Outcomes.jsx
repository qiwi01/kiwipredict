...
const Outcomes = () => {
  ...
  const fetchAvailableDates = async () => {
    try {
      const response = await api.get('/api/outcomes/dates');
  ...
  const getCurrentDisplayData = () => {
    ...
    switch (activeSection) {
      ...
      case 'vip': {
        ...
        return vipData; // Always show VIP section data, regardless of user tier
      }
      default:
                  ...
                  {match.outcomes && match.outcomes.map((outcome, outcomeIndex) => (
                    <div key={outcomeIndex} className="predictions-outcome-item">
                      <div className="predictions-outcome-header">
                        ...
                        {/* VIP Badge for VIP predictions */}
                        {outcome.visibility === 'vip' || outcome.visibility === 'vvip' || outcome.visibility === 'both' ? (
                          <div className={`predictions-vip-badge ${outcome.visibility === 'vvip' ? 'vvip' : 'vip'}`}>
                            <Crown className="predictions-vip-icon" />
                            <span>{outcome.visibility === 'vvip' ? 'VVIP' : 'VIP'}</span>
                          </div>
                        ) : null}
